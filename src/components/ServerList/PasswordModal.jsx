...
function PasswordModal({
  ...
  const savePassword = async (password) => {
    ...
    if (idx !== -1) {
      if (savePass) {
        ...
        servers.passwords = servers.passwords.filter(
          (v) => v.ip !== selected.ip
        );
      }
    } else {
  ...
  return (
    <Modal className="modalCenter" open={open} onClose={handleClose} size="sm">
      <Modal.Header>
      ...
      <Modal.Body>
        <Input
          autoFocus
          placeholder={
            loading ? 'Checking saved passwords...' : 'Server Password'
          }
          value={password}
